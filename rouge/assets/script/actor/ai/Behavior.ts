...
export class MoveToDest extends bt.Action {
    execute(dt: number, result: bt.ExecuteResult) {
       ...
       // console.log("MoveDestDuration",result.blackboard.get(BlackboardKey.MoveDestDuration));
 ...
 * 武器攻击
 */
export class WeaponAttack extends bt.Action {
    weapon: Weapon = null;
    execute(dt: number, result: bt.ExecuteResult) {
        let actor = result.blackboard.get(BlackboardKey.Actor) as Actor;
        let moveDest = result.blackboard.get(BlackboardKey.MoveDest) as Vec3;
        ...
        let distance = dir.length();
        if (!actor) {
            bt.markFail(result); // 如果没有找到actor，标记为失败
            console.log("WeaponAttack: actor is null");
            return;
        }
         // 获取武器组件
        this.weapon = actor.node.getChildByName("weapon").getComponent(Weapon);
         if (!this.weapon) {
             bt.markFail(result); // 如果没有找到武器组件，标记为失败
             console.log("WeaponAttack: weapon is null");
             return;
        }
        if (distance < 50) {
